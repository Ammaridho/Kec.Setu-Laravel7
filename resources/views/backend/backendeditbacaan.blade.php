...
@section('backendcontent')
    ...
    <form class="ml-5 mr-5 text-light pt-3 pl-4 pr-4 pb-2 bg-dark rounded" action="/bacaan/{{$bacaan->id}}" method="POST" enctype="multipart/form-data">
        ...
        <x-textfield field="postedby" label="Postedby" type="text" value="{{$bacaan->postedby}}"/>
        ...
        <div class="form-group mb-3">
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" id="inlineRadio1" value="berita" {{ $bacaan->perihal == "berita" ? 'checked' : '' }} name="perihal">
                <label class="form-check-label" for="inlineRadio1">Berita</label>
            </div>
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" id="inlineRadio2" value="kegiatan" {{ $bacaan->perihal == "kegiatan" ? 'checked' : '' }} name="perihal">
                <label class="form-check-label" for="inlineRadio2">Kegiatan</label>
            </div>
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" id="inlineRadio3" value="artikel" {{ $bacaan->perihal == "artikel" ? 'checked' : '' }} name="perihal">
                <label class="form-check-label" for="inlineRadio3">Artikel</label>
            </div>
        </div>
        
    
        <x-textfield field="judul" label="Judul" type="text" value="{{$bacaan->judul}}"/>

        <x-file field="gambar" label="Gambar" value="{{$bacaan->gambar}}"/>

        <x-textarea field="isi" label="Isi" value="{{$bacaan->isi}}"/>
